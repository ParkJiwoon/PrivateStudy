...
애플리케이션 로직은 크게 **핵심 기능**과 **부가 기능**으로 나눌 수 있습니다.
...
- 부가 기능
  - 핵심 기능을 보조하기 위한 기능
  - 로그 추적, 트랜잭션 등 비즈니스 로직을 감싸고 있는 공통된 로직
  - 모든 비즈니스 로직에 공통으로 들어가기 때문에 **횡단 관심사**라고도 부름

<br>

## 2. 부가 기능 적용 문제

부가 기능은 보통 공통 로직이라서 여러 곳에 적용해야 합니다.

하지만 100 개의 로직이 존재한다면 100 개의 코드를 추가해야하고 별도의 유틸리티 클래스로 공통화해도 결국 중복된 코드는 계속 발생합니다.

- 부가 기능을 적용할 때 많은 반복이 필요하고 중복 코드 발생
- 수정할 때도 중복 때문에 번거로움
...
![](images/screen_2022_03_01_17_17_00.png)

부가 기능을 따로 작성해서 원하는 곳에 일괄적으로 적용하는 기능이 등장했습니다.

`@Aspect` 나 어드바이저도 다 애스펙트(aspect) 의 개념입니다.

애플리케이션을 바라보는 관점을 각각의 로직에서 횡단 관심사로 본다는 의미에서 **관점 지향 프로그래밍 AOP (Aspect-Oriented Programming)** 이라고 합니다.

이러한 기능을 제공하는 걸로 AspectJ 프레임워크가 존재하는데, 스프링 AOP 에서는 그 중 일부 필요한 기능들만 차용해서 제공합니다.

<br>

## 4. AOP 적용

AOP 의 적용 (위빙, Weabing) 시점에는 크게 세가지가 존재합니다.

- **컴파일 시점 (CTW, Compile Time Weaving)**
  - 컴파일 시점 (`.java` 코드를 `.class` 로 만드는 시점) 에 원본 코드에 직접 부가 기능을 붙임
  - AspectJ 가 제공하는 특별한 컴파일러를 사용해야 해서 복잡함
- **클래스 로드 시점 (LTW, Load Time Weaving)**
  - 자바 실행 시 `.class` 파일을 JVM 내부의 클래스 로더에 보관하는 타이밍에 `.class` 파일을 조작하여 직접 부가 기능을 붙임
  - 자바 실행 시 특별한 옵션 (`java -javaagent`) 를 통해 클래스 로더 조작기를 지정해야 해서 번거로움
- **런타임 시점 (프록시) (RTW, Runtime Weaving)**
  - 컴파일도 끝나고 클래스 로더에 전부 올라가서 이미 자바의 `main` 메서드가 실행된 이후 타이밍
  - 스프링과 같은 컨테이너의 도움을 받고 프록시, DI, 빈 포스트 프로세서 등의 도움을 받음
  - 우리가 지금까지 학습한 방식
  - 프록시를 사용하기 때문에 일부 기능에 제약이 있지만 설정이 번거롭지 않고 스프링만 있으면 사용 가능
  - **실제 코드에 추가되는 게 아니라서 항상 프록시를 통해서만 부가 기능을 적용 가능**
...
- **조인 포인트 (Join Point)**
  - 어드바이스 (부가 기능) 가 적용되는 위치, 메서드 실행, 생성자 호출 등 AOP 가 적용될 수 있는 모든 지점 (AspectJ 를 사용하는 경우에 적용가능)
  - 스프링 AOP 는 프록시 방식을 사용하기 때문에 항상 메서드 실행 지점으로만 지정 가능
- **포인트컷 (Pointcut)**
...
`build.gradle` 에 디펜던시를 추가해야 사용할 수 있습니다.

<br>

## 2. 스프링 빈으로 등록

AOP 를 적용하려면 `@Aspect` 어노테이션을 붙인 클래스를 만들고 스프링 빈으로 등록해야 합니다.

- `@Bean` 을 사용해서 등록
- `@Component` 컴포넌트 스캔 사용해서 자동 등록
- `@Import` 로 설정 파일 추가할 때 사용 (`@Configuration`) (테스트 시에 유용)

<br>

## 3. @Pointcut 과 @Aspect 를 사용해서 원하는 클래스와 메서드에 적용

어드바이스 순서를 바꾸려면 Aspect 를 별도의 클래스로 분리한 후에 `@Order(n)` 어노테이션으로 순서를 지정하면 됩니다.

<br>

## 4. 다양한 어드바이스 종류

![](images/screen_2022_03_02_01_35_46.png)
...
- `execution`: 메서드 실행 조인 포인트를 매칭, 스프링 AOP 에서 가장 많이 사용
...
이렇게 종류가 많지만 사실 `execution` 외에는 많이 사용하지 않습니다.
...
`*` 은 모든 값을 매칭한다는 뜻이고 파라미터의 `..` 은 파라미터의 타입과 수가 상관없다는 뜻입니다.
...
`*` 문자가 두개 사용되고 있는데 `(..)` 앞에 있는 것은 메서드를 의미하고 그 전이 패키지 경로를 의미합니다.

패키지에서 `..` 을 사용하는 부분이 있는데 `.` 와 차이가 조금 있습니다.

- `.`: 정확하게 해당 위치의 패키지
- `..`: 해당 위치의 패키지와 그 하위 패키지까지 포함
...
// String 타입으로 시작하고 이후는 모든 타입, 갯수 허용
execution(* *(String, ..))
```
...
JDK 동적 프록시는 인터페이스 기반으로만 생성이 가능하기 때문에 구체 클래스로 타입 캐스팅이 불가능하다는 단점이 있습니다.

CGLIB 프록시는 인터페이스가 없어도 프록시를 생성할 수 있습니다.
...
스프링 4.0 부터는 `objenesis` 라는 특별한 라이브러리를 사용해서 생성자 호출 없이 객체를 생성할 수 있게 해줍니다.

그래서 기본 생성자 필수 및 생성자 두번 호출 하는 이슈가 사라졌습니다.

스프링 부트 2.0 부터는 `proxyTargetClass=true` 옵션이 자동 설정되어 CGLIB 를 기본으로 사용합니다.
